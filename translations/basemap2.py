...
def pointToGrid(x, y, minCoord, maxCoord):
    return int(((x - minCoord[0]) / float(maxCoord[0] - minCoord[0]))*gridLen), int(((y - minCoord[1]) / float(maxCoord[1] - minCoord[1]))*gridLen)
...
def preOutputTransformDisabled(geometries, features):
    ...
    for feat in features:
        if isinstance(feat.geometry, Point) and "building" in feat.tags:
            pt = feat.geometry
            if minCoord[0] is None or pt.x < minCoord[0]:
                minCoord[0] = pt.x
            if minCoord[1] is None or pt.y < minCoord[1]:
                minCoord[1] = pt.y
            if maxCoord[0] is None or pt.x > maxCoord[0]:
                maxCoord[0] = pt.x
    ...
    for feat in features:
        if isinstance(feat.geometry, Point) and "building" in feat.tags:
            pt = feat.geometry
            gridX, gridY = pointToGrid(pt.x, pt.y, minCoord, maxCoord)
    ...
    for i, geo in enumerate(geometries):
        ...
        if isinstance(geo, Way):
            ...
            minY = min(geo.points, key=lambda a: a.y)
            maxY = max(geo.points, key=lambda a: a.y)
            minGridX, minGridY = pointToGrid(minX.x, minY.y, minCoord, maxCoord)
            maxGridX, maxGridY = pointToGrid(maxX.x, maxY.y, minCoord, maxCoord)
            ...
            containedPoints = set()
            for y in range(minGridY, min(maxGridY+1, gridLen)):
                for x in range(minGridX, min(maxGridX+1, gridLen)):
                    for point in points[y][x]:
                        if "building" not in point.tags:
                            continue
                        ogrPoint = ogr.Geometry(ogr.wkbPoint)
                        ogrPoint.AddPoint(point.geometry.x, point.geometry.y)
                        if ogrPoly.Contains(ogrPoint):
                            containedPoints.add(point)
            if len(containedPoints) > 1:
                for point in containedPoints:
                    del point.tags["building"]
